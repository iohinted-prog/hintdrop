...
function getImageCandidates($, base = "") {
  ...
  function add(raw) {
    ...
    const abs = upgradeAsosImageResolution(makeAbsolute(String(raw || "").trim(), base));
...
function getImage($, base = "") {
  ...
  for (const candidate of candidates) {
    const abs = upgradeAsosImageResolution(makeAbsolute(candidate, base));
    if (abs) return abs;
  }
...
function mapLinkPreviewResult(inputUrl, payload, preferredCurrency = "GBP") {
  ...
  const image = upgradeAsosImageResolution(String(payload?.image || "").trim());
...
// Amazon's product-image CDN doesn't just serve static photos — the
// star-rating badge seen baked into some images (e.g. "4.8" plus a
