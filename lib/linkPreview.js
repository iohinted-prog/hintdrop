...
import { filterImagesWithoutText } from "./textDetection";
...
function getImageCandidates($, base = "") {
  ...
  function add(raw) {
    ...
    const abs = makeAbsolute(String(raw || "").trim(), base);
...
async function stripImagesWithText(result) {
  const candidates = result.imageCandidates?.length
  ...
  const survivors = await filterImagesWithoutText(candidates);
  const hadImage = Boolean(result.image);

  result.imageCandidates = survivors;
  result.image = survivors[0] || "";
  result.selectedImage = result.image;

  if (hadImage && !result.image) {
    // Every candidate turned out to have text on it (a marketing
    // banner/badge, not a clean product photo) - nothing left to show
    // automatically, so this needs the person to review/upload one
    // themselves, same as any other "couldn't find a usable image"
    // case.
...
export async function getProductPreview(rawUrl, currency = "GBP") {
  ...
  if (!isBlockedRetailer(inputUrl)) {
    try {
      ...
      if (isUsablePreview(htmlResult)) {
        return await stripImagesWithText(htmlResult);
      }
    } catch (err) {
      htmlError = err;
    }
  ...
  try {
    ...
    if (
      isAmazonUrl(inputUrl) &&
      linkPreviewResult.image &&
      !isLikelyRealAmazonProductImage(linkPreviewResult.image)
    ) {
      linkPreviewResult.image = "";
    ...
    if (isUsablePreview(linkPreviewResult)) {
      ...
      return await stripImagesWithText(linkPreviewResult);
    }
