...
function getImageCandidates($, base = "") {
  const seen = new Set();
  const candidates = [];
...
const BLOCKED_RETAILERS = [
  ...
  // sites (several of these - eBay, Alibaba, AliExpress, Shein, Temu -
  // are well known for aggressive anti-bot measures at the CDN/WAF
  // level, and thewhitecompany.com specifically looks like a
  // client-rendered app whose static HTML shell may not carry
  // per-product data at all, independent of bot-blocking). Worth
  // re-testing each of these for real once deployed, since "routed
  // through LinkPreview instead" and "actually produces a good image"
  // are two different claims.
  "endclothing.com", "traderjoes.com", "ebay.com", "ebay.co.uk",
  "alibaba.com", "office.co.uk", "jdsports.co.uk", "aliexpress.com",
...
function parseHtmlPreview({ html, finalUrl, status, preferredCurrency }) {
  ...
  const titleCandidates = [ogTitle, twitterTitle, h1, titleTag].filter(Boolean);
  const title =
  ...
  const imageCandidates = getImageCandidates($, finalUrl);
