...
// Security) - confirmed as the actual cause of some scraped hint
// images silently failing to load in the mobile app specifically
// (web/Android aren't affected the same way). Most sites serving
// product images support https too even when the page itself
// happened to link the http version, so upgrading here is safe far
// more often than not - this is a browser-fetched image URL, not
// user-entered data that needs preserving exactly as given.
...
export async function getProductPreview(rawUrl, currency = "GBP", options = {}) {
  ...
  if (result?.image) result.image = upgradeToHttps(result.image);
  if (Array.isArray(result?.imageCandidates)) {
    result.imageCandidates = result.imageCandidates.map(upgradeToHttps);
  }
