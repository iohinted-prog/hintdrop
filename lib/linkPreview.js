...
const BLOCKED_RETAILERS = [
  ...
  "net-a-porter.com",
  // Reported failing (logo-only, never scrapes, or a generic/unrelated
  // image regardless of the actual product URL) - added here so the
  // direct-fetch attempt (already confirmed not working for these) is
  // skipped in favor of LinkPreview.net, rather than continuing to try
  // and fail every time. This is NOT a confirmed fix - LinkPreview.net
  // may also struggle with heavily bot-protected or client-rendered
  // sites (several of these - eBay, Alibaba, Shein, Temu - are well
  // known for aggressive anti-bot measures at the CDN/WAF level, and
  // thewhitecompany.com specifically looks like a client-rendered app
  // whose static HTML shell may not carry per-product data at all,
  // independent of bot-blocking). Worth re-testing each of these for
  // real once deployed, since "routed through LinkPreview instead" and
  // "actually produces a good image" are two different claims.
  //
  // aliexpress.com deliberately NOT here (was, briefly, removed after
  // testing) - a real fetch through a full headless-browser/proxy
  // service reached the actual product page cleanly, no CAPTCHA/block
  // page, unlike Shein which was confirmed redirected to a CAPTCHA
  // wall on the same kind of test. Being on this list means skipping
  // our own direct fetch entirely in favor of LinkPreview.net, which
  // would also skip the JSON-LD extraction added for exactly this kind
  // of case - since AliExpress appears genuinely reachable, it should
  // get a real shot at our own scraper instead of being routed around
  // it by default.
  "endclothing.com", "traderjoes.com", "ebay.com", "ebay.co.uk",
  "alibaba.com", "office.co.uk", "jdsports.co.uk",
  "three.co.uk", "temu.com", "thewhitecompany.com",
  "mercadolibre.com", "mercadolibre.com.mx", "mercadolibre.com.ar",
  "shein.com", "hm.com", "zara.com", "boots.com", "argos.co.uk",
  "laredoute.co.uk", "laredoute.fr",
];
...
export async function getProductPreview(rawUrl, currency = "GBP") {
