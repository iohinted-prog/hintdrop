...
async function login(email, password) {
  ...
  await browser.storage.local.set({ hintdrop_session: session });
  return session;
}

async function getStoredSession() {
  const { hintdrop_session } = await browser.storage.local.get("hintdrop_session");
...
async function logout() {
  await browser.storage.local.remove("hintdrop_session");
}
